...
describe('AtendimentoService', () => {
  ...
  it('deve chamar HttpClient ao executar o método get()', () => {
    ...
    const req = httpTestingController.expectOne('http://localhost:9000/atendimento/');
    expect(req.request.method).toBe('GET');
  });

  it('deve chamar HttpClient ao executar o método get(termoBusca)', () => {
    service.get('teste').subscribe();
    const req = httpTestingController.expectOne('http://localhost:9000/atendimento/busca/teste');
    expect(req.request.method).toBe('GET');
  });

  it('deve chamar HttpClient ao executar o método getById()', () => {
    service.getById(1).subscribe();
    const req = httpTestingController.expectOne('http://localhost:9000/atendimento/1');
    expect(req.request.method).toBe('GET');
  });
